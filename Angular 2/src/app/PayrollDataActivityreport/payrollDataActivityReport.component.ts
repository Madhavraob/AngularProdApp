...
export class PayrollDataActivityReportComponent implements OnInit {
    ...
    selectedControlGroups: string[];
    dataLoaded: boolean;
    //these go into options
...
//          this.Years = data.WorkYears;
            ...
            data.WorkYears.forEach(element =>{
...
//      this.controlGroupControl.setValue('');
    ...
    getFilterValues(): any {
        ...
        let cg = this.selectedControlGroups;
        if ( cg === undefined) {
                cg = [];
                cg[0] = '\'\'';
                this.selectedControlGroups=[];
