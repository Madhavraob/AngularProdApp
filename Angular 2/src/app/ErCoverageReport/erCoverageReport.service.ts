...
export class ErCoverageReportService {
    ...
    getCoverageByYear(filterCriteria: any): Observable<any> {
        
        let fileName: string = 'getERCoverageFTECountByWorkMonth?WorkMonth='+ filterCriteria.selectedYear;
        return this._http.get(this._erCoverageReportUrl + fileName)
            .map((response: Response) => response.json().fteCountByWorkMonthList)
